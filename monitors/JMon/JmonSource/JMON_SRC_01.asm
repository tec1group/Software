...
; VERSION : 1
;
; RESTORED TO SOURCE FROM PDF LISTING(S) FEBRUARY 2021
...
;THIS IS ENCODED IN BITS 2 AND 3. IF NO FUNCTION OR FUNCTION-1 IS ENABLED THjN BOTH BITS 
...
L082C   EQU  082CH                      ;DISPLAY BUFFER ADDRESS

L082E   EQU  082EH                      ;CURRENT EDITING LOCATION
...
L0851   EQU  0851H
L0858   EQU  0858H                      ;"NEXT PC" BUFFER   
L085A   EQU  085AH
...
L0086  LD HL,0x070F    ;LOAD HL WITH START OF JMON DEFAULT
...
L00FD  LD DE,0x0845    ;BUT RETURN ADDRESS ON STACK
       PUSH DE
...
L0150  LD A,B          ;PUT MONITOR CONTROL BYTE IN A
       ...
       LD HL,0x08BE    ;LOAD HL WITH USER TABLE -2
...
L0305  ADD A, (HL)     ;ADD CHECKSUM
       ...
       LD HL,0x08B5    ;POINT TO REQUIRED CHECKSUM 
...
L0312  LD (L0870),HL   ;STORE HL IN ITS REGISTER STACK SPOT
       LD HL,0x0344    ;LOAD HL WITH RETURN ADDRESS
...
; <PAGE 11>
       ...
       LD SP,0x087E    ;"NEXT PC" BUFFER: LOAD REGISTER DUMP
       ...
       LD SP,0x086A    ;SHIFT 7 ROUTINE START (REG DISPLAY)
...
;BUFFER.
       ...
       LD HL,0x0806    ;CREATE NEW DISPLAY BUFFER
       LD (L082C),HL   ;
...
L038A  LD B,A          ;SAVE KEY
       ...
       LD HL,0x085A    ;THUS DISABLING THE AUTO REPEAT MODE
...
L03F9  PUSH HL         ;SAVE ADDR POINTER (not required) 
       ...
       LD DE,0x0800    ;SHIFT ACROSS ADDR DISPLAY
...
;BEING A 3 BYTE JUMP THEN JUMPS TO THE SELECTED MENU ENTRY'S ROUTINE.
       ...
       CALL   L0842    ;CALL SCAN/KEY/LCD/PATCH ROUTINE
       LD HL,0x088F    ;POINT HL TO MENU ENTRY NUMBER
...
;IN THE ADDRESS SECTION OF THE DISPLAY BUFFER.
       ...
       LD BC,0x0800    ;BC=DISPLAY BUFFER START
...
L05B4  LD DE,0x0768    ;LOAD DE TO BASE OF DATA DISPLAY 
L05B7  LD HL,0x075C    ;TABLE AND HL "FAIL" DISPLAY 
       JR NZ,L05BE     ;TABLE:
...
L069F  LD HL,0x0752    ;POINT HL TO "Err-In" DISPLAY
       LD DE,0x0800    ;CODE AND DE TO RAM DEstination
...
L06D5  LD HL,0x07B5    ;POINT HL TO LCD INITIALIZE TABLE 
...
;0700  C3  3F 04             ;HIGH SPEED SAVE
;0703  C3  3F 04             ;LOW SPEED SAVE
;0706  C3  3B 04             ;TEST BLOCK
;0709  C3  37 04             ;TEST CHECKSUM
;070C  C3  26 04             ;LOAD TAPE
...
;070F 00                     ;KEY BUFFER                                                0820
;0710 00                     ;LCD ON/OFF FLAG                                           0821*
;0711 00                     ;SOUND ON/OFF                                              0822*
;0712 FF                     ;GO AT ALTERNATE GO ADDRESS IF AA                          0823*
;0713 FF                     ;STEPPER KEY CONTROL/TIMER                                 0824 
;0714 00                     ;KEY PRESSED FLAG                                          0825 
;0715 FF                     ;UNUSED                                                    0826
;0716 00                     ;AUTO INCREMENT ON/OFF                                     0827*
;0717 00 OA                  ;ALT GO ADDR/SOFT RESET EDIT LOCATION                      0828*
;0719 70                     ;AUTO KEY REPEAT TIMER                                     082A
;071A 00                     ;MONITOR CONTROL BYTE                                      082B
;071B 00 08                  ;DISPLAY BUFFER ADDRESS                                    082C*
;071D 00 09                  ;INITIAL EDITING LOCATION                                  082E
...
;071F C3 D5 01              ;CONVERT HL TO DISPLAY CODE                               0830
;0722 C3 DA 01              ;CONVERT A TO DISPLAY CODE                                0833
;0725 C3 BA 01              ;LED SCAN ROUTINE                                         0836
;0728 C3 EE 01              ;SET LED DOTS                                             0839
;072B C3 24 02              ;RESET TONES                                              083C
;072E C3 27 02              ;TONE                                                     083F
;0731 C3 81 01              ;SCAN/KEY/LCD/PATCH LOOP                                  0842
;0734 C3 B2 00              ;SOFT JMON ENTRY                                          0845
;0737 C3 3C 02              ;LCD ROUTINE                                              0848
...
;073A   A7 6F EA C7           ;"SAVE" 
;073E   A7 6F EA C7           ;"SAVE" 
;0742   C6 C7 A7 C6           ;"TEST" 
;0746   C6 C7 A7 C6           ;"TEST" 
;074A   C2 EB  6F EC          ;"LOAD" 
;074E   04 C7  64 EC          ;"-End" 
;0752   04 C7  44 44  28 64   ;"-Err In"
;0758  4F 6F A7 A7            ;"PASS' 
;075C  47 6F  28 C2           ;"FAIL

       .DB   0xA7,0x6F,0xEA,0xC7 
       .DB   0xA7,0x6F,0xEA,0xC7
       .DB   0xC6,0xC7,0xA7,0xC6 
       .DB   0xC6,0xC7,0xA7,0xC6 
       .DB   0xC2,0xEB,0x6F,0xEC
       .DB   0x04,0xC7,0x64,0xEC  
       .DB   0x04,0xC7,0x44,0x44,0x28,0x64
       .DB   0x4F,0x6F,0xA7,0xA7
       .DB   0x47,0x6F,0x28,0xC2 
...
;077C 00 FF C6 07 99 08 00 03 ;(FF FF; THE JUMP ADDRESS FOR THE TAPE 
...
;0789 FF FF 00 04 00 07 3A 07 60 07
...
;07AD 44 61 74 61    "Data"
;07B1 41 64 64 72    "Addr"
L07AD  .DB 0x44,0x61,0x74,0x61   
L07B1  .DB 0x41,0x64,0x64,0x72 
...
L07D0  .DB 0xEB,0x28,0xCD,0xAD,0x2E,0xA7,0xE7,0x29,0xEF,0x2F,0x6F,0xE6,0xC3,0xEC,0xC7,0x47

;FINALLY AT 07E0 IS THE FUNCTION-1 AND SHIFT JUMP ADDRESSES.
;07E0 D2 03 E3 02 5E 00 FF FF D3 02 AE 00 DE 02 41 03 
;07F0 ED 02 E8 02 F2 02 FF FF FF FF FF FF FF FF FF FF
